...
function N(eq::KleinGordon, v)
    ...
    space_N = Taylor(3order(space(space(v))))^1
...
function f(eq::KleinGordon, w)
    ...
    space_f = Chebyshev(3order(space(space(w))))^3
...
function Df(eq::KleinGordon, w)
    w₁, w₂, w₃ = eachcomponent(w)

    n_C = order(space(space(w)))
    domain_Df = Chebyshev(3n_C + 1)^3
    codomain_Df = Chebyshev(n_C + 1)^3
