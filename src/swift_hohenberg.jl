...
function N(eq::SwiftHohenberg, v)
    ...
    space_N = Taylor(3order(space(space(v))))^2
...
function DN(eq::SwiftHohenberg, v)
    v₁, v₂ = eachcomponent(v)
...
function f(eq::SwiftHohenberg, w)
    ...
    space_f = Chebyshev(3order(space(space(w))))^5
...
function Df(eq::SwiftHohenberg, w)
    w₁, w₂, w₃, w₄, w₅ = eachcomponent(w)

    n_C = order(space(space(w)))
    domain_Df = Chebyshev(3n_C + 1)^5
    codomain_Df = Chebyshev(n_C + 1)^5
