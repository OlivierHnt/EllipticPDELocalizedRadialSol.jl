...
function __prove(eq, x, n_T_pad, n_C_pad, c, Λ, Γ, l, r_star, L, ℒ_x, ℒ_y, ν, ϱ)
    ...
    if isLPcontracting(eq, c, Λ, Γ, λ̂, δ, μ, ℒ_x, ℒ_y)
        ...
        F_ = F(x, eq, c, Λ, Γ, l, r_star, L)

        space_x_ext = ParameterSpace()^q × ParameterSpace()^q × Taylor(n_T)^q × Chebyshev(n_C)^(1+2q)
        x_ext = project(x, space_x_ext)
        DF_ = DF(x_ext, eq, c, Λ, Γ, l, r_star, L)

        tail_N = copy(component(F_, 3))
        tail_f = copy(component(F_, 4))
        for i ∈ 1:q
            component(tail_N, i)[0:min(n_T, order(component(tail_N, i)))] .= 0
            component(tail_f, i)[0:min(n_C, order(component(tail_f, i)))] .= 0
            component(tail_f, i+q+1)[0:min(n_C, order(component(tail_f, i+q+1)))] .= 0
        end

        A = inv(mid.(project(DF_, space_x_ext, space_x_ext)))
        ...
        Z₁ = opnorm(A * DF_ - I, X) +
                ...
                max(
                    2 / ν^(n_C_max + 1),
                    r_star^(n_T + 1) * max(1, (n_T + 1) / l) + L * opnorm_Df_ / (ν^(n_C + 2) * ((n_C + 2)^2 - 1))
                )
...
function F(x, eq, c, Λ, Γ, l, r_star, L)
    ...
    F_ = Sequence(space_F, zeros(T, dimension(space_F)))
    ...
    for i ∈ 1:q
        ...
        for n ∈ 2:n_T_max
            component(F₃, i)[n] = l^2 * component(N_, i)[n-2]
            if n ≤ n_T
    ...
    F₄ = component(F_, 4)
    rsub!(mul!(F₄, Integral(1), f_, L/2, false), w)
    component(F₄, 1)[0] += inv(l*r_star)
    for i ∈ 1:q, n ∈ 0:n_T
        component(F₄, i+1)[0] += component(v, i)[n] * r_star^n
        component(F₄, i+q+1)[0] += n * component(v, i)[n] * r_star^n / l
    end
...
function DF(x, eq, c, Λ, Γ, l, r_star, L)
    v, w = component(x, 3), component(x, 4)
    q = nspaces(space(v))
    ...
    domain_DF = ParameterSpace()^q × ParameterSpace()^q × space(v) × domain(Df_)
    DF_ = LinearOperator(domain_DF, space(x), zeros(T, length(x), dimension(domain_DF)))
    ...
    for j ∈ 1:q, i ∈ 1:q
        if i == j
            ...
            for m ∈ 0:n_T, n ∈ 2:n_T
                component(D₃F₃, i, i)[n,m] = l^2 * component(DN_, i, i)[n-2,m]
                if n == m
            ...
            for m ∈ 0:n_T, n ∈ 2:n_T
                component(D₃F₃, i, j)[n,m] = l^2 * component(DN_, i, j)[n-2,m]
            end
        end
    end
    ...
    for i ∈ 1:q, n ∈ 0:n_T
        component(D₃F₄, i+1, i)[0,n] = r_star^n
        component(D₃F₄, i+q+1, i)[0,n] = n * r_star^n / l
    end
